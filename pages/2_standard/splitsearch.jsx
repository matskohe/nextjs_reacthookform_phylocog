  ...
  return (
    ...
    <h2>Split Search</h2>
      <form onSubmit={handleSubmit(onSubmit)}>
        <div>
          <input type="checkbox" {...register('split_iq')} />
          <label htmlFor="split_iq">IQ Tree</label>
        </div>

        <div>
          <input type="checkbox" {...register('split_fast')} />
          <label htmlFor="split_iq">Fast Tree</label>
        </div>

        {(watchShowSplitIq || watchShowSplitFast) && (
          <div>
            <p>split between</p>
            <input
              type="number"
        ...
        {(watchShowSplitIq || watchShowSplitFast) && <input type="submit" />}
      </form>
    </>
  );
};
